...
contract Random is ERC721, VRFConsumerBaseV2 {
    ...
    /**
     * 使用chainlink VRF,构造函数需要继承VRFConsumerBaseV2
     * 不同链参数填的不一样
     * 网络: Speolia测试网
        ...
        returns (uint256 tokenId)
    {
        ...
        //随机数取模,得到tokenId,作为数字下标,同时记录value为len-1
    ...
    //生成链上伪随机数,返回时转换成uint256类型
    //keccak256(abi.encodePacked())中填上一些链上的全局变量/自定义变量法
    ...
    //调用vRF获取随机数,并mintNFT
    //要调用requestRandomness()函数,消耗随机数的逻辑写在VRF的回调函数fulfillRandomness()中
    //调用前,需要在Subsriptions中fund足够的Link
    ...
    //VRF的回调函数,由VRF Voordinator调用
    //消耗随机数的逻辑写在本函数中
    function fulfillRandomWords(uint256 _requestId, uint256[] memory s_randomWords)
        internal
        override
    {
